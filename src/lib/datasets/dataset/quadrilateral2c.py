...
class Quadrilateral2c(data.Dataset):
    num_classes = 2
    num_edges = 4
    ...
    def __init__(self, opt, split):
        super(Quadrilateral2c, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'cuadros')
        self.img_dir = os.path.join(self.data_dir, '{}'.format(split))  # os.path.join(self.data_dir, '{}'.format(split))
        self.annot_path = os.path.join(self.data_dir, 'cuadros_{}.json'.format(split))
        self.max_objs = 1000
        self.class_name = [
